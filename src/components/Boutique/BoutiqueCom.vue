<template>
  <div
      ...
      :style="`background: url(${coverImgUrl});background-size: 300%;background-position: 100% 50%;`"
    >
      <div
        class="my-2 p-6 flex items-center relative z-20 rounded-2xl"
        style="backdrop-filter: blur(60px)"
      >
        <img class="w-48 h-48 rounded-xl" :src="coverImgUrl" alt />
        <div class="ml-4 flex flex-col mb-10">
          <span
            class="mb-4 py-2 w-36 rounded-full flex justify-center items-center"
            style="border: 1px solid #fdcb6e; color: #fdcb6e"
          >
            <svg
              t="1672298155864"
              class="icon w-6 h-6"
...
export default defineComponent({
  setup () {
    ...
    const init = async () => {
      ...
      coverImgUrl.value = res.playlists[0].coverImgUrl
      coverName.value = res.playlists[0].name
    }
